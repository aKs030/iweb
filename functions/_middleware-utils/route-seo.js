import { loadJsonFile, resolveOrigin, toAbsoluteUrl } from "../api/_xml-utils.js";
import {
  formatSlug,
  humanizeSlug,
  normalizeText,
  ...
  normalizePathname,
} from "../../content/core/utils/index.js";
...
function buildStaticPageMeta(requestUrl, pathname, config) {
  ...
  const schema =
    typeof config.schema === "function"
      ...
      : config.schema || null;
  const partialMeta =
    config.partialMeta && typeof config.partialMeta === "object" ? { ...config.partialMeta } : null;
  ...
  return {
    ...
    twitterTitle: normalizeText(config.twitterTitle, normalizeText(config.ogTitle, title)),
...
async function loadVideoDetails(context, videoId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (item) {
        const requiredChannelId = normalizeText(context.env?.YOUTUBE_CHANNEL_ID);
        const itemChannelId = normalizeText(item?.snippet?.channelId);
        if (!requiredChannelId || !itemChannelId || requiredChannelId === itemChannelId) {
          value = item;
        }
      }
    }
...
async function buildBlogMeta(context, requestUrl, postId) {
  ...
  const image = toAbsoluteUrl(origin, normalizeText(post.image, DEFAULT_BLOG_IMAGE));
  const imageAlt = sanitizeDiscoveryText(post.imageAlt, `${postTitle} - Artikelbild`);
...
async function buildProjectAppMeta(context, requestUrl, appSlug) {
  ...
  const appTitle = sanitizeDiscoveryText(app.title, humanizeSlug(appName) || formatSlug(appName));
  ...
  const image = normalizeText(app?.previewUrl) || buildProjectPreviewUrl(app) || DEFAULT_APP_IMAGE;
  const tags = Array.isArray(app.tags)
    ? app.tags.map(entry => normalizeText(entry)).filter(Boolean)
    : [];
  const keywords = [...new Set([appTitle, "Projekte", "Web App", ...tags])].join(", ");

  const appType = normalizeText(app.category, "WebApplication");
  const schemaType = appType.toLowerCase() === "game" ? "GameApplication" : "SoftwareApplication";
...
async function buildVideoMeta(context, requestUrl, videoId) {
  ...
  const description = clampText(
    sanitizeDiscoveryText(snippet.description, `${title} - Video von Abdulkerim Sesli`),
    220
  );
...
export async function buildRouteMeta(context, requestUrl) {
  ...
  if (STATIC_ROUTE_META[staticRouteKey]) {
    return buildStaticPageMeta(requestUrl, staticRouteKey, STATIC_ROUTE_META[staticRouteKey]);
  }
