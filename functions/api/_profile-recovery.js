...
import { normalizeSchemaText } from "../../content/core/utils/index.js";
...
function getLatestNameValue(memories = []) {
  ...
  for (const entry of memories) {
    if (normalizeSchemaText(entry?.key || "").toLowerCase() !== NAME_MEMORY_KEY) continue;

    const name = normalizeSchemaText(entry?.value || "");
    if (!name) continue;

    const timestamp = Math.max(0, Number.parseInt(String(entry?.timestamp), 10) || 0);
...
function summarizeMemories(memories = []) {
  ...
  for (const entry of memories) {
    ...
    if (!value) continue;
    count += 1;
    latestMemoryAt = Math.max(latestMemoryAt, Number.parseInt(String(entry?.timestamp), 10) || 0);
  }
...
async function loadRecoveryCandidatesFromKv(
  ...
  do {
    ...
    for (const key of keys) {
      ...
      const userId = normalizeUserId(keyName.slice(FALLBACK_MEMORY_PREFIX.length));
      if (!userId) continue;
...
export function pickAutoRecoveryCandidate(candidates = []) {
  ...
  const nonTrivial = ordered.filter(candidate => Number(candidate?.memoryCount || 0) > 1);
  if (nonTrivial.length === 1) return nonTrivial[0] || null;

  const [top, second] = ordered;
  if (top && second && Number(top.memoryCount || 0) >= 3 && Number(second.memoryCount || 0) <= 1) {
    return top;
  }
