...
import { CACHE_CONTROL_PRIVATE_NO_STORE, mergeHeaders } from "../_shared/http-headers.js";
...
function jsonCorsResponse(corsHeaders, payload, status = 200, extraHeaders = {}) {
...
function errorCorsResponse(corsHeaders, error, status = 500, extraHeaders = {}) {
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      return errorCorsResponse(corsHeaders, "Content-Type must be application/json", 400);
    }
    ...
    if (!name || !email || !message) {
      return errorCorsResponse(corsHeaders, "Bitte füllen Sie alle Pflichtfelder aus.", 400);
    }
    ...
    if (!resendResponse.ok) {
      return errorCorsResponse(corsHeaders, "Fehler beim Senden der E-Mail.", 500);
    }
    ...
    return errorCorsResponse(corsHeaders, "Ein unerwarteter Fehler ist aufgetreten.", 500);
  }
}
