let fallbackRateLimitTokenCounter = 0;
...
function createRateLimitToken() {
  ...
  fallbackRateLimitTokenCounter = (fallbackRateLimitTokenCounter + 1) % Number.MAX_SAFE_INTEGER;
  return `${Date.now().toString(36)}${fallbackRateLimitTokenCounter.toString(36)}`;
}
...
export function createWindowRateLimiter(options) {
  const cleanupIntervalMs = Math.max(1, Number(options?.cleanupIntervalMs || 300_000));
  const keyNamespace = String(options?.keyNamespace || "rl:v2").trim() || "rl:v2";
  const maxEntries = Math.max(100, Number(options?.maxEntries || 1000));
  const memoryStore = new Map();
  let lastCleanupAt = Date.now();

  function maybeCleanupStore(now) {
    if (memoryStore.size <= maxEntries && now - lastCleanupAt < cleanupIntervalMs) {
      return;
    }
