...
async function executeServerTool(env, toolName, args, userId, config) {
  ...
  if (toolName === "recallMemory") {
    const memories = await recallMemories(env, userId, args.query || "", config);
    if (!memories.length) return "Keine Erinnerungen gefunden.";
    return "Bekannte Infos:\n" + memories.map(m => `- **${m.key}**: ${m.value}`).join("\n");
  }
...
export async function classifyToolCalls(env, toolCalls, userId, config) {
  ...
  for (const tc of toolCalls) {
    ...
    const serverResult = await executeServerTool(env, tc.name, args, userId, config);
