...
import { escapeHtml, escapeRegExp, normalizeForMatch } from "../../content/core/utils/text-utils.js";
...
const STATIC_SEARCH_RESULTS = Object.freeze([
  {
    title: "Startseite",
    url: "/",
    description: "Portfolio-Startseite mit 3D-Visualisierung, Projekten und AI-Funktionen.",
    category: "Home",
  ...
  },
  {
    title: "Über mich",
    url: "/about/",
    description: "Profil, Tech-Stack und beruflicher Hintergrund von Abdulkerim Sesli.",
    category: "Über mich",
  ...
  },
  {
    title: "Projekte",
    url: "/projekte/",
    description: "Übersicht interaktiver Web-Apps, Games, Tools und Experimente.",
    category: "Projekte",
...
function buildHighlightedDescription(description, queryTokens) {
  ...
  const regex = new RegExp(`(${safeTokens.map(token => escapeRegExp(token)).join("|")})`, "gi");
...
function rerankSearchResults(results, query) {
  ...
  return (Array.isArray(results) ? results : []).map(item => {
    ...
    const highlightedDescription = buildHighlightedDescription(
      [description, item?.title || "", getSearchableKeywordText(item)].filter(Boolean).join(" "),
      queryTokens
    );
...
function isTechnicalResult(url) {
  ...
  return TECHNICAL_RESULT_PREFIXES.some(prefix => normalized.startsWith(prefix));
}
...
function getResultFacet(result) {
  ...
  if (category === "projekte" || url === "/projekte/" || url.startsWith("/projekte/")) {
    return "projects";
  }
  if (category === "videos" || url === "/videos/" || url.startsWith("/videos/")) {
    return "videos";
  }
  return "pages";
}
...
function buildAiSuggestions(query, results, facet = "all", maxSuggestions = AI_SUGGESTION_LIMIT) {
  const limit = Math.max(1, Number.parseInt(String(maxSuggestions || 0), 10) || 0);
  const suggestions = [];
  ...
  const fallbackSeed = STATIC_SEARCH_RESULTS.map(item => buildDeterministicResult(item));
  const rankedFallback = rerankSearchResults(fallbackSeed, query).sort((a, b) => b.score - a.score);
  const facetedFallback = filterResultsByFacet(rankedFallback, facet);
  const fallbackCandidates = facetedFallback.length > 0 ? facetedFallback : rankedFallback;
...
function buildAllowedResultPathSet(results) {
  return new Set(
    (Array.isArray(results) ? results : []).map(item => normalizeUrl(item?.url)).filter(Boolean)
  );
}
...
export async function loadDeterministicSearchDataset(context) {
  ...
  deterministicSearchCachePromise = (async () => {
    ...
    deterministicSearchCacheExpiresAt = Date.now() + DETERMINISTIC_SEARCH_CACHE_TTL_MS;

    if (kv && context.waitUntil) {
      context.waitUntil(
        kv
          .put(CACHE_KEY, JSON.stringify(dataset), {
...
export function resolveSearchInput(request, body = null) {
  ...
  return {
    query: String(
      body?.query ?? url.searchParams.get("q") ?? url.searchParams.get("query") ?? ""
    ).trim(),
    topK: body?.topK ?? url.searchParams.get("topK") ?? url.searchParams.get("limit"),
    facet: body?.facet ?? url.searchParams.get("facet") ?? url.searchParams.get("tab"),
  };
}
...
export function buildSearchPayload({
  ...
  const rerankedResults = rerankSearchResults(Array.from(uniqueResultsMap.values()), query).sort(
    (a, b) => b.score - a.score
  );
