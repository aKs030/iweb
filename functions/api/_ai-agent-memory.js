...
async function recallMemoriesFromFallback(
  ...
  return entries
    ...
    .sort((a, b) => (b.score === a.score ? b.timestamp - a.timestamp : b.score - a.score))
    .slice(0, topK);
}
...
export async function storeMemory(env, userId, key, value, config, precomputedEmbedding = null) {
  ...
  if (!env.AI || !env.JULES_MEMORY) {
    return kvStored ? { success: true, id: `kv_${Date.now()}`, storage: "kv" } : { success: false };
  }
...
export async function recallMemories(env, userId, query, config, options = {}) {
  ...
  try {
    ...
    if (!error?.remote) log.warn("recallMemories error:", error?.message || error);
    return recallMemoriesFromFallback(env, userId, query, {
      topK,
      scoreThreshold,
    });
...
export async function persistPromptMemories(env, userId, prompt, config, sharedEmbedding = null) {
  ...
  return facts
    ...
    .filter(entry => entry.result.status === "fulfilled" && entry.result.value?.success)
    .map(entry => ({
      key: entry.fact.key,
...
export async function resolveMemoryContext(env, userId, prompt, config, sharedEmbedding = null) {
  ...
  if (needsRecall) {
    const recallQueries = ["name", "interests", "preferences", "notes about this user"];
...
export function mergeMemoryEntries(recalled = [], stored = []) {
  ...
  const seen = new Set(merged.map(entry => `${entry.key}:${String(entry.value).toLowerCase()}`));
