...
export async function storeMemory(
	...
	try {
		...
		const embedding =
			precomputedEmbedding ?? (await computeEmbedding(env, text, config));
		if (!embedding) {
			return kvStored
				? { success: true, id: `kv_${Date.now()}`, storage: "kv" }
...
export async function recallMemories(env, userId, query, config, options = {}) {
	...
	try {
		...
		const embedding =
			options.embedding ?? (await computeEmbedding(env, query, config));
		if (!embedding) {
			return recallMemoriesFromFallback(env, userId, query, {
				topK,
				scoreThreshold,
 ...
 * @returns {Promise<object[]>}
 */
export async function persistPromptMemories(
	env,
	userId,
	prompt,
	config,
	...
	const results = await Promise.allSettled(
		facts.map((fact) =>
			storeMemory(env, userId, fact.key, fact.value, config, sharedEmbedding),
		),
	);
...
export async function resolveMemoryContext(
	...
	if (needsRecall || hasExtractedFacts) {
		const primary = await recallMemories(
			env,
			userId,
			prompt || "user",
			config,
