...
export const sseEvent = (event, data) => `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;

export function buildAgentMessagePayload(text, clientToolCalls, serverToolResults, ctx, config) {
...
export async function streamToSSE(stream, write) {
  ...
  const consumeChunk = async (chunk, isFinal = false) => {
    ...
    const tail = lineBuffer.endsWith("\r") ? lineBuffer.slice(0, -1) : lineBuffer;
