...
function getAgentConfig(env) {
  return {
    ...
    maxMemoryResults: parseInteger(env.ROBOT_MEMORY_TOP_K, DEFAULT_MAX_MEMORY_RESULTS, {
      min: 1,
      max: 25,
    }),
    ...
    maxHistoryTurns: parseInteger(env.ROBOT_MAX_HISTORY_TURNS, DEFAULT_MAX_HISTORY_TURNS, {
      min: 1,
      max: 40,
    }),
...
async function requestClientToolConfirmationText(env, messages, clientToolCalls, config) {
  const result = await env.AI.run(
    config.chatModel,
    buildAiRunParams(buildClientToolConfirmationMessages(messages, clientToolCalls), config, {
      maxTokens: 256,
    })
  );
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      body = {
        prompt: String(formData.get("prompt") || "") || "Analysiere dieses Bild.",
    ...
    const [memResult, storedPromptMemoriesResult] = await Promise.allSettled([
    ...
    const recalledMemories = memResult.status === "fulfilled" ? memResult.value : [];
    const storedPromptMemories =
      storedPromptMemoriesResult.status === "fulfilled" ? storedPromptMemoriesResult.value : [];
    const mergedMemories = mergeMemoryEntries(recalledMemories, storedPromptMemories);
    ...
    const write = (event, data) => writer.write(encoder.encode(sseEvent(event, data)));
    ...
    context.waitUntil(
      (async () => {
        try {
          ...
          let toolCalls = Array.isArray(aiResult?.tool_calls) ? aiResult.tool_calls : [];
...
async function processToolCalls(
  ...
  if (serverToolResults.length > 0 && messages.length > 0) {
    ...
    const followUpMessages = buildServerToolFollowUpMessages(messages, serverToolResults);
    try {
      const result = await env.AI.run(
        config.chatModel,
      ...
      if (hasMeaningfulAgentText(text)) {
        await write(
          "message",
          buildAgentMessagePayload(text, clientToolCalls, serverToolResults, ctx, config)
        );
        return;
      }
  ...
  if (clientToolCalls.length > 0 && !hasMeaningfulAgentText(existingText) && messages.length > 0) {
    ...
    try {
      ...
      if (followUpText) {
        ...
        await write(
          "message",
          buildAgentMessagePayload(followUpText, clientToolCalls, serverToolResults, ctx, config)
        );
        return;
      }
    } catch (err) {
...
async function handleNonStreaming(env, messages, userId, corsHeaders, ctx, config) {
  try {
    const useTools = promptNeedsTools(messages[messages.length - 1]?.content || "");
    const aiResult = await env.AI.run(
      config.chatModel,
      buildAiRunParams(messages, config, { useTools })
    );
    ...
    let toolCalls = Array.isArray(aiResult.tool_calls) ? aiResult.tool_calls : [];
    if (useTools && toolCalls.length === 0) {
      toolCalls = inferClientToolCallsFromPrompt(messages[messages.length - 1]?.content || "");
    }
    ...
    if (serverToolResults.length > 0) {
      try {
        const followUp = await env.AI.run(
          config.chatModel,
          buildAiRunParams(buildServerToolFollowUpMessages(messages, serverToolResults), config)
        );
    ...
    return jsonResponse(
      buildAgentResponsePayload({
        text:
          responseText || (clientToolCalls.length ? "Aktion wird ausgeführt…" : "Keine Antwort."),
