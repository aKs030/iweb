...
export function resolveMemoryMetadata(key, category, priority) {
  ...
  const finalPriority = Number.isFinite(parsedPriority) ? parsedPriority : metadata.priority;
...
function compactMemories(entries, now, retentionMs) {
  ...
  for (const rawEntry of entries) {
    ...
    if (
      ...
      (entry.timestamp === existing.timestamp && entry.priority > existing.priority)
    ) {
