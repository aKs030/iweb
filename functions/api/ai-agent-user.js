...
function getLatestMemoryEntry(entries, key) {
  ...
  for (const entry of entries || []) {
    ...
    if (
      ...
      (entry.timestamp === latestEntry.timestamp && entry.priority > latestEntry.priority)
    ) {
      latestEntry = entry;
    }
  }
...
export function buildProfileInfo(userId, memories = []) {
  const name = normalizeMemoryText(getLatestMemoryEntry(memories, "name")?.value);
...
export async function deleteUserProfile(
  ...
  const [usernameStats, vectorizeStats] = shouldDeferVectorize
    ? await Promise.all([usernameStatsPromise, Promise.resolve(buildDeferredVectorizeStats(env))])
    : await Promise.all([usernameStatsPromise, deleteVectorizeMemoriesForUser(env, userId)]);
...
async function upsertVectorizeMemory(env, userId, entry) {
  ...
  try {
    ...
    const vector = Array.isArray(embeddingResult?.data) ? embeddingResult.data[0] : null;
...
async function deleteVectorizeMemoriesForUser(env, userId) {
  ...
  const metadataDeleteMethod = await tryDeleteVectorizeByMetadata(
    ...
    { userId },
    (method, error) => {
      log.warn(`[ai-agent-user] Vectorize ${method} failed:`, error?.message || error);
    }
  );
  ...
  try {
    ...
    log.warn(`[ai-agent-user] Vectorize ${idDeleteMethod} failed:`, error?.message || error);
    return {
      available: true,
      attempted: true,
      deleted: 0,
...
async function loadUserMemorySnapshot(kv, env, userId) {
  ...
  return {
    memories,
    previousName: normalizeMemoryText(getLatestMemoryEntry(memories, "name")?.value),
  };
}
...
export async function updateSingleMemory(kv, env, userId, body) {
  ...
  const { memories: existing, previousName } = await loadUserMemorySnapshot(kv, env, userId);
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const headerUserId = normalizeUserId(request.headers.get(USER_ID_HEADER_NAME));
    ...
    if (action === "resolve") {
      const recoveryName = normalizeMemoryText(body?.name || body?.recoveryName || "");
      if (!recoveryName) {
        return jsonResponse(
          {
            success: false,
      ...
      return jsonResponse(
        {
          ...
          recovery,
          text:
            candidates.length > 0 ? "Profile gefunden." : "Kein Profil für diesen Namen gefunden.",
        },
    ...
    if (action === "activate") {
      const targetUserId = normalizeUserId(body?.targetUserId || body?.candidateUserId);
      const recoveryName = normalizeMemoryText(body?.name || body?.recoveryName || "");
      ...
      return jsonResponse(
        await buildMemoryListPayload(kv, env, targetUserId, {
          ...
          emptyText: "Profil aktiviert. Es sind noch keine Erinnerungen gespeichert.",
        }),
        { headers: responseHeaders }
      );
    }
    ...
    if (action === "list") {
      return jsonResponse(await buildMemoryListPayload(kv, env, userId), {
        headers: responseHeaders,
      });
    }
    ...
    if (action === "update-memory") {
      ...
      return jsonResponse(buildMemoryMutationPayload(result, userId), {
