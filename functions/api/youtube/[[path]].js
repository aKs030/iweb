...
const ALLOWED_ENDPOINTS = new Set(["channels", "playlistItems", "search", "videos"]);
...
function buildResponseHeaders(request, env, extraHeaders = {}) {
  return mergeHeaders(getSecurityHeaders(), getCorsHeaders(request, env), extraHeaders);
}

function buildJsonResponse(request, env, payload, status = 200, extraHeaders = {}) {
...
function sanitizeYouTubeParams(endpoint, searchParams, env) {
  ...
  if (endpoint === "channels") {
    const channelId = String(searchParams.get("id") || expectedChannelId || "").trim();
  ...
  if (endpoint === "playlistItems") {
    ...
    params.set("maxResults", String(normalizeMaxResults(searchParams.get("maxResults"))));
    params.set("playlistId", playlistId);
    return { params };
  }

  if (endpoint === "search") {
    const channelId = String(searchParams.get("channelId") || expectedChannelId || "").trim();
    if (!CHANNEL_ID_PATTERN.test(channelId)) {
      return { error: "Invalid channel id." };
    }
    if (expectedChannelId && channelId !== expectedChannelId) {
...
export async function onRequestGet(context) {
  ...
  if (!endpoint || !ALLOWED_ENDPOINTS.has(endpoint)) {
    return buildJsonResponse(request, env, { error: "Invalid YouTube API endpoint." }, 400, {
  ...
  try {
    ...
    return buildJsonResponse(request, env, { error: "YouTube API request failed" }, 502, {
      "Cache-Control": CACHE_CONTROL_NO_STORE,
    });
  }
}
