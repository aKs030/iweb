...
function clampResults(value, fallback = DEFAULT_MAX_SEARCH_RESULTS) {
  ...
  const candidate = Number.isFinite(parsed) ? parsed : Number.parseInt(String(fallback), 10);
...
async function handleSearchRequest(context, body = null) {
  ...
  const { query, topK: requestedTopK, facet: requestedFacet } = resolveSearchInput(request, body);
