...
function normalizeNameCandidate(candidate) {
  ...
  if (selected.length === 1 && NON_NAME_SINGLE_WORDS.has(selected[0].toLowerCase())) {
    return "";
  }
...
export function inferClientToolCallsFromPrompt(prompt) {
  ...
  if (/(scroll|geh).*(nach oben|ganz nach oben|top)|\bscroll top\b|seite nach oben/i.test(text)) {
    addTool("scrollTop");
  }
  ...
  if (/(öffn|oeffn|zeige).*(suche|search)|(suche|search).*(öffn|oeffn|auf)/i.test(text)) {
    addTool("openSearch");
  }

  const searchMatch = text.match(/(?:such(?:e)?|finde?)\s+(?:nach\s+)?(.{2,120})$/i);
  if (searchMatch?.[1]) {
  ...
  const sectionMatch = text.match(/(header|footer|kontakt|contact|hero|projects|projekte|skills)/i);
  if (/(scroll|geh|spring)/i.test(text) && sectionMatch?.[1]) {
    const section = sectionMatch[1].toLowerCase();
    addTool("scrollToSection", {
      section: section === "kontakt" ? "contact" : section === "projekte" ? "projects" : section,
    });
  }

  if (/(geh|navigier|zeige|öffn|oeffn).*(about|über|profil)/i.test(text)) {
    addTool("navigate", { page: "about" });
  } else if (/(geh|navigier|zeige|öffn|oeffn).*(projekt|projekte|apps)/i.test(text)) {
  ...
  } else if (/(geh|navigier|zeige|öffn|oeffn).*(kontakt|contact|footer)\b/i.test(text)) {
