...
function parseWebpDimensions(bytes) {
  ...
  if (chunkType === "VP8L" && bytes.length >= 25) {
    ...
    const height = 1 + ((((bytes[22] >> 6) | (bytes[23] << 2) | (bytes[24] << 10)) & 0x3fff) >>> 0);
    return { width, height };
  }
...
export async function enrichGalleryObject(bucket, obj) {
  ...
  const dimensions = type === "image" ? await probeImageDimensions(bucket, key) : null;
  const orientation = inferOrientation(dimensions?.width, dimensions?.height, type);
  ...
  return {
    ...
    presentationMeta: {
      ...
      width: Number.isFinite(dimensions?.width) ? Math.round(dimensions.width) : null,
      height: Number.isFinite(dimensions?.height) ? Math.round(dimensions.height) : null,
    },
  };
}
...
export function buildGalleryItemPayload(obj, url) {
  ...
  const presentationMeta =
    obj.presentationMeta && typeof obj.presentationMeta === "object" ? obj.presentationMeta : {};
  ...
  return {
    ...
    dominantColor: String(presentationMeta.dominantColor || derivePreviewColor(key)),
    blurPlaceholder: String(
      presentationMeta.blurPlaceholder || createBlurPlaceholder(derivePreviewColor(key), title)
    ),
    width: Number.isFinite(presentationMeta.width) ? presentationMeta.width : null,
    height: Number.isFinite(presentationMeta.height) ? presentationMeta.height : null,
  };
}
