...
function inferProxyContentType(pathname, contentTypes, sourceHeaders) {
  ...
  return mappedType || sourceHeaders?.get("content-type") || "application/octet-stream";
}
...
export function buildProxyResponseHeaders(options) {
  ...
  headers.set(
    "Content-Type",
    inferProxyContentType(options.pathname, options.contentTypes, options.sourceHeaders)
  );
  headers.set("Cache-Control", options.cacheControl || DEFAULT_PROXY_CACHE_CONTROL);
 ...
 */
export function maybeBuildNotModifiedResponse(request, headers, cacheHeaderName, cacheStatus) {
 ...
 * @returns {Headers}
 */
export function buildConditionalProxyRequestHeaders(request, extraHeaders = {}) {
