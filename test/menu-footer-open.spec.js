...
test.describe('Menu footer open', () => {
  test('clicking nav "Kontakt" should open the maximized footer', async ({ page }) => {
    ...
    const contactAnchor = await page.waitForSelector('#navigation a[href="#site-footer"]', { state: 'visible', timeout: 5000 });
    ...
    await page.evaluate((sel) => { const el = document.querySelector(sel); if (el) el.click(); }, '#navigation a[href="#site-footer"]');
