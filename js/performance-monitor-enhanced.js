...
class WebsitePerformanceMonitor {
    ...
    setupCoreWebVitals() {
        if ('PerformanceObserver' in window) {
            try {
                ...
                console.warn('Some PerformanceObserver features not supported:', error.message);
            }
        }
    }
    ...
    trackScrollPerformance() {
        ...
        const trackScrollFrame = () => {
            if (scrollStart) {
                ...
                if (elapsed > 1000) { // Track for 1 second
                    let rating;
                    if (fps >= 55) {
                        rating = 'good';
                    } else if (fps >= 30) {
    ...
    trackMenuPerformance() {
        document.addEventListener('click', (event) => {
            if (event.target.closest('.menu-toggle') || event.target.closest('.menu-item')) {
    ...
    analyzePerformanceTrends() {
        const criticalMetrics = ['LCP', 'FID', 'CLS', 'templateLoad', 'memoryUsage'];
...
if (window.location.hostname === 'localhost') {
    window.debugPerformance = {
        ...
        alert: (type, data) => window.websitePerformanceMonitor.dispatchPerformanceAlert(type, data)
    };
    
    console.log('Debug functions: debugPerformance.summary(), debugPerformance.metrics(name), debugPerformance.all()');
}
