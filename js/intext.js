...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addCardLayoutClasses(containerElement) {
        ...
        if (!containerElement?.parentElement || containerElement.parentElement.id !== 'section-features') {
            return;
        }
    ...
    function processAnimationElement(element, cardElements, staggerDelayText, staggerDelayCard, textDelayRef) {
    ...
    function handleScrollToSection(event) {
        try {
            ...
            if (sectionElement && shuffledSections[sectionId]) {
                ...
                const viewportBox = document.querySelector('.viewport-box');
                if (!viewportBox) {
                    console.warn('Viewport-Box nicht gefunden für ScrollToSection Observer');
                    return;
                }
            ...
            console.error('Fehler beim Verarbeiten des scrollToSection Events:', error);
        }
    }
    ...
    function handleTouchEvent(event, action) {
        try {
            ...
            console.error(`Fehler beim Verarbeiten des Touch-Events (${action}):`, error);
        }
    }

    // Registriere Touch-Events mit optimierter Event-Delegation
    document.addEventListener('touchstart', (event) => handleTouchEvent(event, 'add'), { passive: true });
    document.addEventListener('touchend', (event) => handleTouchEvent(event, 'remove'), { passive: true });
    document.addEventListener('touchcancel', (event) => handleTouchEvent(event, 'remove'), { passive: true });
