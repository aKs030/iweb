...
const loadLatestVideos = async () => {
  ...
  try {
    ...
    const featuredVideo = targetVideoId
      ? extractVideoViewModel(
          items.find(entry => entry?.snippet?.resourceId?.videoId === targetVideoId),
          detailsMap
        )
      : null;
    const relatedVideos = featuredVideo
      ? items
          ...
          .filter(entry => entry && entry.id !== featuredVideo.id && entry.thumb)
          .slice(0, 3)
      : [];
    ...
    AppLoadManager.updateLoader(0.6, i18n.t("videos.processing", { count: items.length }));
    ...
    AppLoadManager.updateLoader(0.95, i18n.t("loader.videos_loaded", { count: items.length }));
    setVideoStatus("");
