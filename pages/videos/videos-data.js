...
export async function loadFromApi(options = {}) {
  ...
  const notifyInfo = typeof options.notifyInfo === "function" ? options.notifyInfo : null;
  ...
  if (uploads) {
    ...
    if (items.length === 0) {
      if (isLocalDevRuntime()) {
        log.info("Uploads playlist returned no items in local dev — falling back to search");
      } else {
        log.warn("Uploads playlist returned no items — falling back to search");
      }
      notifyInfo?.("Uploads playlist leer — lade Videos per Suche als Fallback.");
      items = await searchChannelVideos(channelId);
    }
  } else {
    if (isLocalDevRuntime()) {
      log.info("No uploads playlist available in local dev — falling back to search");
    } else {
      log.warn("No uploads playlist available — falling back to search");
    }
    notifyInfo?.("Uploads playlist nicht vorhanden — lade Videos per Suche als Fallback.");
    items = await searchChannelVideos(channelId);
  }

  if (!items.length) return { items: [], detailsMap: {} };

  const vidIds = items.map(item => item.snippet.resourceId.videoId).filter(Boolean);
...
export async function ensureDeepLinkedVideo(options = {}) {
  ...
  const detailsMap =
    options.detailsMap && typeof options.detailsMap === "object" ? { ...options.detailsMap } : {};

  if (!videoId) return { items, detailsMap };

  const exists = items.some(item => item.snippet?.resourceId?.videoId === videoId);
  if (exists) {
    return { items, detailsMap };
  }
