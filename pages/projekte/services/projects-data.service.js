...
import { fetchJSON, normalizeText } from "#core/utils/index.js";
...
const TECH_STACK_RULES = Object.freeze([
  ...
  {
    label: "Game Logic",
    keywords: ["game", "spiel", "puzzle", "quiz", "memory", "snake", "pong", "tic"],
  },
  {
    label: "Local Storage",
...
function normalizeList(values) {
  ...
  return [...new Set(input.map(value => normalizeText(value)).filter(Boolean))];
}
...
function findSharedLabel(sourceValues, candidateValues) {
  const lookup = new Map(normalizeList(sourceValues).map(entry => [entry.toLowerCase(), entry]));
...
function getRelatedMatch(sourceProject, candidateProject) {
  const sharedStack = findSharedLabel(sourceProject?.techStack, candidateProject?.techStack);
  if (sharedStack) {
    return { type: "stack", label: sharedStack };
  }

  const sharedTag = findSharedLabel(sourceProject?.tags, candidateProject?.tags);
  if (sharedTag) {
...
async function loadLocalConfig() {
  ...
  localConfigPromise = (async () => {
    try {
      localAppsConfig = await fetchJSON("/pages/projekte/apps-config.json", {
        retries: 1,
      });
    } catch (error) {
...
function getProjectIconAndTheme(project, icons) {
  ...
  for (const [categoryKey, categoryData] of Object.entries(PROJECT_CATEGORIES)) {
    if (categoryKey === "default") continue;

    const matches = categoryData.keywords.filter(keyword => allText.includes(keyword)).length;
...
function mapProjectRecord(project, index, icons, fallbackDate) {
  ...
  const { icon } = getProjectIconAndTheme(project, icons);
...
function scoreRelatedProject(sourceProject, candidateProject) {
  ...
  const sourceTags = new Set(normalizeList(sourceProject.tags).map(tag => tag.toLowerCase()));
...
export async function createProjectsData(icons) {
  ...
  const fallbackDate =
    normalizeDateStamp(config?.lastUpdated) || new Date().toISOString().split("T")[0];
  ...
  AppLoadManager.updateLoader(1, i18n.t("loader.projects_ready", { count: projects.length }));
