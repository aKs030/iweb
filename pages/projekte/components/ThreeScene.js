...
const log = createLogger("ProjectsThreeScene");
...
function getRuntimeQualitySignals() {
  const nav =
    typeof navigator === "undefined" ? /** @type {any} */ ({}) : /** @type {any} */ (navigator);
...
function getProjectsSignature(projects) {
  return projects.map(project => project?.name || project?.dirName || project?.id || "").join("|");
}
...
function maybeDowngradeQuality(now) {
  ...
  if (runtime.frameDeltas.length < 12 || now - runtime.lastPerformanceCheckTime < 2000) {
    return false;
  }

  runtime.lastPerformanceCheckTime = now;
  const avgDelta =
    runtime.frameDeltas.reduce((sum, value) => sum + value, 0) / runtime.frameDeltas.length;
  runtime.frameDeltas = [];
...
export const ThreeScene = ({ projects, onScrollUpdate, onReady }) => {
  ...
  useEffect(() => {
    ...
    if (!globalQualityProfile) {
      setActiveQualityProfile(resolveInitialQualityProfileKey(window.innerWidth));
    }
    ...
    if (!globalRenderer) {
      try {
        ...
        log.error("Three.js Init Failed:", e);
        return;
      }
    }
    ...
    if (
      ...
      globalRenderer.domElement.parentElement.removeChild(globalRenderer.domElement);
    }
    if (globalRenderer.domElement.parentElement !== containerRef.current && containerRef.current) {
    ...
    const doResize = () => {
      if (!isMountedRef.current || !globalRenderer || !globalCamera || !containerRef.current)
        return;
      ...
      globalRenderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, quality.dprCap));
    ...
    const animate = time => {
      ...
      if (
        ...
        time - globalQualityRuntime.lastRenderTime < quality.targetFrameIntervalMs
      ) {
        return;
      }
      ...
      if (globalCamera && globalCameraLight) {
        ...
        const totalPathLength = globalGallery?.getPathLength?.() || projects.length * 15 + 10;
        const targetZ = 5 - scrollRef.current * totalPathLength;

        // More responsive camera movement
        const lerpFactor = motion.lerpFactor;
        globalCamera.position.z += (targetZ - globalCamera.position.z) * lerpFactor;

        // Reduce lateral motion on small screens to keep focus centered.
        globalCamera.position.x = Math.sin(t * 0.5) * motion.xAmplitude * quality.motionScale;
        globalCamera.position.y =
          motion.yBase + Math.cos(t * 0.3) * motion.yAmplitude * quality.motionScale;
      ...
      if (globalStars) {
        const viewportWidth = viewportWidthRef.current;
        const motion = getMotionProfile(viewportWidth);
        globalStars.rotation.z = t * motion.starRotationSpeed * quality.motionScale;
      }

      // Update Gallery
      if (globalGallery) {
        globalGallery.update(t);
        const activeIndex = globalGallery.getActiveIndex(globalCamera.position.z);
