...
(() => {
  ...
  function drawParticle(ctx, particle, progress, time) {
    ...
    const twinkle = (Math.sin(time * STARFIELD_CONFIG.TWINKLE_SPEED + particle.twinkleOffset) + 1) / 2;
  ...
  function animateReverseStarfield(section) {
    ...
    starfieldParticles.forEach(particle => {
      ...
      // Twinkle-Effekt
      const twinkle = (Math.sin(now / 1000 * STARFIELD_CONFIG.TWINKLE_SPEED + particle.twinkleOffset) + 1) / 2;
  ...
  function observe() {
    ...
    io = new IntersectionObserver(
      (ents) => {
        for (const e of ents) {
          ...
          log.debug(`📊 Intersection: visible=${isVisible}, ratio=${ratio}, hasAnimated=${hasAnimated}, isReversing=${isReversing}`);

          // Reverse Animation: Section verlässt Viewport
          if (hasAnimated && !isReversing && (!isVisible || parseFloat(ratio) < REVERSE_THRESHOLD)) {
    ...
    const handleScroll = () => {
      ...
      const visibleRatio = Math.max(0, Math.min(rect.bottom, viewportHeight) - Math.max(rect.top, 0)) / rect.height;
