...
import { createUseTranslation, sanitizeHTML } from "#core/utils/index.js";
...
import { ProgressiveImage, ScrollToTop, BlogLikes } from "./components/BlogComponents.js";
...
const BlogApp = () => {
  ...
  const openPost = React.useCallback(post => {
    ...
    if (window.location.pathname !== nextPath || String(window.location.hash || "").length > 0) {
  ...
  const renderPostContent = post => {
    ...
    try {
      ...
      return html`<p className="blog-inline-error">${t("blog.render_error")}</p>`;
    }
  };
        ...
        <article className="blog-article">
          ...
          <header>
            ...
            <div className="meta">
              ...
              <span className="card-read-time"> <${Clock} /> ${activePost.readTime} </span>
          ...
          post => html`
            <article key=${post.id} className="blog-card" onClick=${() => openPost(post)}>
              ...
              <div className="card-content-wrapper">
                ...
                ${post.excerpt && html`<p className="card-excerpt">${post.excerpt}</p>`}
