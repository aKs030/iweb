...
const setCommonMeta = (title, description, url, image, imageAlt, keywords) => {
  ...
  setMeta('meta[property="og:image:type"]', "property=og:image:type", inferImageType(image));
...
const createStructuredData = (post, url, image, desc, keywords, body) => {
  ...
  return {
    ...
    inLanguage: "de-DE",
    author: [{ "@type": "Person", name: "Abdulkerim Sesli", url: `${BLOG_BASE_URL}/` }],
    publisher: {
      "@type": "Organization",
...
export const updatePostMeta = post => {
  ...
  const structuredData = createStructuredData(post, url, image, desc, keywords, body);
