...
function normalizeGalleryItem(item) {
  ...
  return {
    ...
    description,
    caption,
    orientation: String(item.orientation || (type === "video" ? "landscape" : "unknown")),
    dominantColor: String(item.dominantColor || "#111827"),
    blurPlaceholder: String(item.blurPlaceholder || ""),
    width: Number.isFinite(item.width) && Number(item.width) > 0 ? Number(item.width) : null,
    height: Number.isFinite(item.height) && Number(item.height) > 0 ? Number(item.height) : null,
  };
}
  ...
  useEffect(() => {
    ...
    const initGallery = async () => {
      try {
        ...
        AppLoadManager.updateLoader(0.2, i18n.t("gallery.loading.init") || "Initializing...");

        // Fetch dynamic items from R2 API
        let dynamicItems = [];
        try {
          const data = await fetchJSON("/api/gallery-items", {
          ...
          log.warn("Failed to load dynamic gallery items, falling back to static config", apiErr);
        }
        if (isCancelled) return;

        // Use dynamic items if available, otherwise fallback to static config
        const finalItems = dynamicItems.length > 0 ? dynamicItems : STATIC_GALLERY_ITEMS;
        setItems(finalItems.map(item => normalizeGalleryItem(item)).filter(Boolean));

        AppLoadManager.updateLoader(0.5, i18n.t("gallery.loading.prepare") || "Preparing Scene...");
        if (isCancelled) return;

        AppLoadManager.updateLoader(0.9, i18n.t("gallery.loading.assets") || "Loading Assets...");
        if (isCancelled) return;

        AppLoadManager.updateLoader(1, i18n.t("gallery.loading.ready") || "Ready");
        ...
        setItems(STATIC_GALLERY_ITEMS.map(item => normalizeGalleryItem(item)).filter(Boolean));
