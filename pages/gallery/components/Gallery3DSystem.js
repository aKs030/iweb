...
export class Gallery3DSystem {
  ...
  createGalleryObjects() {
    ...
    const geometry = new THREE.PlaneGeometry(this.params.itemWidth, this.params.itemHeight);
  ...
  onMouseMove(e) {
    if (e.clientX !== undefined) {
      ...
      if (Math.abs(this.mouse.x - newX) > 0.001 || Math.abs(this.mouse.y - newY) > 0.001) {
        this.mouse.x = newX;
        this.mouse.y = newY;
  ...
  onClick() {
    ...
    const intersects = this.raycaster.intersectObjects(this._raycastMeshes || []);
  ...
  handleResize() {
    ...
    if (this._prevWidth && Math.abs(currentWidth - this._prevWidth) < 10) return;
  ...
  animate() {
    ...
    this.camera.rotation.y =
      -(this.scrollPos / this.params.verticalStep) * this.params.rotationStep - Math.PI / 2;
    ...
    this.objects.forEach(obj => {
      ...
      const targetScaleVec =
        obj.mesh === this.hoveredMesh ? this._hoverScaleVec : this._normalScaleVec;
