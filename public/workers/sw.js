...
// Version: 2026-01-21 - v6 with complete LocalAI implementation

// Cache names with versioning for better cache busting
const CACHE_NAMES = {
  APP_SHELL: 'aks-portfolio-app-v6', // Bumped version to clear old caches
  RUNTIME: 'aks-portfolio-runtime-v6',
...
self.addEventListener('activate', (event) => {
  ...
  event.waitUntil(
    caches
      ...
      .then(() => {
        // Force clear any cached files that might contain old references
        return Promise.all([
            ...
            return cache.keys().then((keys) => {
              return Promise.all(
                keys
                  .filter(
                    (key) =>
                      key.url.includes('legacy-ai') ||
                      key.url.includes('robot-companion') ||
                      key.url.includes('local-ai-service'),
                  )
...
self.addEventListener('fetch', (event) => {
  ...
  // Handle legacy AI endpoint redirects to prevent 404s
  if (
    url.pathname.includes('/legacy-ai') ||
    url.pathname.includes('old-ai-service')
  ) {
    event.respondWith(
      new Response('', {
        status: 301,
    ...
    event.respondWith(
      caches.match(req).then((cached) => {
        ...
        return fetch(req)
          ...
          .catch((_error) => {
            // Log texture loading errors for debugging
            return new Response('', {
              status: 404,
              statusText: 'Texture not found',
            });
          });
      }),
    );
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.action === 'clearCache') {
    Promise.all([
      caches.delete(CACHE_NAMES.APP_SHELL),
      caches.delete(CACHE_NAMES.RUNTIME),
      caches.delete(CACHE_NAMES.IMAGES),
      caches.delete(CACHE_NAMES.TEXTURES),
      caches.delete(CACHE_NAMES.FONTS),
      caches.delete(CACHE_NAMES.BUNDLES),
    ])
      .then(() => {
