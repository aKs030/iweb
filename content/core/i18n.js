...
/**
 * @typedef {Record<string, string|number|boolean|null|undefined> & { count?: number }} TranslationParams
 */
...
class LanguageManager extends EventTarget {
  ...
  constructor() {
     ...
     * @public
     */
    this.translations = Object.fromEntries(SUPPORTED_LANGUAGES.map(lang => [lang, null]));
  ...
  async init() {
    ...
    this.loadingPromise = (async () => {
      ...
      document.documentElement.lang = this.currentLang === "de" ? "de-DE" : "en-US";
   ...
   * @param {TranslationParams} [params] - Optional parameters for interpolation and pluralization.
   ...
   * Supports `data-i18n` for text content, `data-i18n-html` for sanitized HTML,
  ...
  translateElement(element) {
    ...
    const allI18nEls = element.querySelectorAll
      ? Array.from(element.querySelectorAll("[data-i18n], [data-i18n-html], [data-i18n-attrs]"))
      : [];
    ...
    for (const el of allI18nEls) {
      try {
        ...
        // 2. Sanitized HTML: data-i18n-html
        const htmlKey = el.getAttribute("data-i18n-html");
        if (htmlKey) {
          const translated = this.t(htmlKey);
          setSanitizedHTML(el, translated, {
            ALLOWED_TAGS: [
              "b",
              "i",
              "em",
