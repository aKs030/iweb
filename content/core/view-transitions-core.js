...
const hasActiveBackdropEffect = style => {
  ...
  const webkit =
    /** @type {{ webkitBackdropFilter?: string }} */ (style).webkitBackdropFilter || "";
...
const shouldSkipForLiveBackdrop = (types, options) => {
  ...
  if (!types.some(type => BACKDROP_SENSITIVE_TYPES.has(/** @type {any} */ (type)))) {
    return false;
  }
...
const startTransition = (update, types) => {
  if (types.length && supportsTypedTransitions()) {
    const transitionInit = /** @type {any} */ ({
      update,
      types,
    });
    return document.startViewTransition(transitionInit);
  }
...
export async function withViewTransition(callback, options = {}) {
  ...
  await runInTransitionQueue(async () => {
    try {
      ...
      const typedTransition = /** @type {ViewTransition & { types?: Set<string> }} */ (transition);
