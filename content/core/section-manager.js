...
const buildSectionSwapTypes = sectionId => {
  ...
  return token
    ? [VIEW_TRANSITION_TYPES.SECTION_SWAP, `${VIEW_TRANSITION_TYPES.SECTION_SWAP}-${token}`]
...
export class SectionManager {
  ...
  async loadSection(section) {
    if (this.loadedSections.has(section) || this.loadingSections.has(section)) return;
    ...
    try {
      ...
      const renderJob = (this._renderQueue || Promise.resolve()).then(async () => {
        ...
        try {
          await withViewTransition(
            () => {
              ...
              section.append(...Array.from(parsed.body.childNodes));
        ...
        if (wasSnapPage) {
          ...
          this._snapRestorer = setTimeout(() => htmlEl.classList.add("snap-page"), 150);
        }
      });
  ...
  init() {
    ...
    document.querySelectorAll("section[data-section-src]").forEach(section => {
