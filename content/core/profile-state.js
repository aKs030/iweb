...
/**
 * @typedef {{ userId?: unknown, name?: unknown, status?: unknown, label?: unknown, [key: string]: unknown }} ProfileStateInput
 * @typedef {{ userId: string, name: string, status: string, label: string }} ProfileState
 * @typedef {{
 *   userId?: unknown,
 *   name?: unknown,
 *   status?: unknown,
 *   memoryCount?: unknown,
 *   latestMemoryAt?: unknown,
 *   source?: unknown,
...
/**
 * @param {ProfileStateInput} [overrides]
 * @returns {ProfileState}
 */
export function createProfileState(overrides = {}) {
  const userId = normalizeUserId(overrides.userId);
  const name = normalizeSchemaText(String(overrides.name || ""));
  const status = normalizeProfileStatus(
    String(overrides.status || (name ? "identified" : userId ? "anonymous" : "disconnected"))
  );
  const label =
    normalizeSchemaText(String(overrides.label || "")) || buildProfileLabel(status, name);

  return { userId, name, status, label };
}

/**
 * @param {RecoveryCandidateInput|null|undefined} raw
 * @param {{ source?: unknown }} [options]
 * @returns {RecoveryCandidate|null}
 */
export function normalizeRecoveryCandidate(raw, { source } = {}) {
  ...
  const candidate = {
    userId,
    name: normalizeSchemaText(String(raw?.name || "")),
    status: normalizeProfileStatus(String(raw?.status || "anonymous")),
    memoryCount: parseNonNegativeInteger(raw?.memoryCount),
    latestMemoryAt: parseNonNegativeInteger(raw?.latestMemoryAt),
  };

  const candidateSource = normalizeSchemaText(String((source ?? raw?.source) || ""));
  if (candidateSource) {
...
export function createRecoveryState(raw = {}, { allowNone = false, limit = 5 } = {}) {
  ...
  return {
    ...
    candidateUserId: normalizeUserId(raw?.candidateUserId) || candidates[0]?.userId || "",
    autoCandidateUserId: normalizeUserId(raw?.autoCandidateUserId) || "",
    candidates,
  };
}
 ...
 * @param {{ fallbackUserId?: string, currentProfile?: ProfileStateInput }} [options]
 */
export function resolveProfileStatePayload(
  payload = {},
  { fallbackUserId = "", currentProfile = {} } = {}
) {
  const { hasExplicitUserId, userId } = resolvePayloadUserId(payload, fallbackUserId);
  let profile = userId
    ? createProfileState({
        userId,
