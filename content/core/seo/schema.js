...
import { applyCspNonce } from "../utils/csp-nonce.js";
import { createLogger } from "../logger.js";
import { scheduleIdleTask } from "../utils/async-utils.js";
...
export function generateSchemaGraph(pageData, pageUrl, brandData, options = {}) {
  ...
  if (deduped.length !== graph.length) {
    log.debug(`Schema graph deduplicated: ${graph.length} -> ${deduped.length}`);
  }
...
export function injectSchema(graph, options = {}) {
  ...
  try {
    ...
    let script = /** @type {HTMLScriptElement} */ (doc.getElementById(scriptId));
