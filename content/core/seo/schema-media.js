...
import { normalizeSchemaText as normalizeText } from "../utils/text-utils.js";
...
export function buildImageObject({
  ...
  if (encodingFormat) /** @type {any} */ (imageNode).encodingFormat = encodingFormat;

  const finalDimensions = dimensions || inferImageDimensions(absoluteUrl);
  if (finalDimensions?.width) /** @type {any} */ (imageNode).width = finalDimensions.width;
  if (finalDimensions?.height) /** @type {any} */ (imageNode).height = finalDimensions.height;
  if (representativeOfPage) /** @type {any} */ (imageNode).representativeOfPage = true;
