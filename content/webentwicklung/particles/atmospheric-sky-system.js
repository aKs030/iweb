...
// ===== Parallax-Effekt beim Scrollen (vereinfacht für Section-based system) =====
function setupParallaxEffect(background) {
  log.debug('Setting up parallax effect');
  
  handleParallax = throttle(() => {
    try {
      // Vereinfachter Parallax-Effekt nur für sanfte Übergänge
...
function createAtmosphericHTML(background) {
  log.debug('Creating atmospheric HTML structure');
  ...
  const html = `
    ...
    <!-- Landschafts-Silhouetten mit mehreren Ebenen -->
    <div class="landscape">
