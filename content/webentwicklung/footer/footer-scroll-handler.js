...
function initializeFooterScrollHandler() {
  ...
  const checkFooterReady = () => {
    ...
    if (footer) {
      ...
      log.info('Footer Scroll Handler initialisiert');
    } else {
...
function setupLastSectionObserver() {
  ...
  if (!triggerZone || !footer) {
    log.warn('Footer-Trigger-Zone oder Footer nicht gefunden für Observer');
    return;
  }
  ...
  log.info('Footer Trigger Zone Observer eingerichtet');
}
...
function toggleFooterExpansion(shouldExpand) {
  ...
  if (!footer || !footerMinimized || !footerMaximized) {
    log.warn('Footer-Elemente nicht gefunden für Toggle');
    return;
  }
...
function cleanup() {
  if (lastSectionObserver) {
    ...
    log.info('Footer Scroll Observer cleanup');
  }
}
