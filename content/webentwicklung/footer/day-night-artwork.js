...
class DayNightArtworkManager {
  ...
  async initialize() {
    try {
      log.debug('Initialisiere Day/Night Artwork...');
      ...
      if (!this.artwork) {
        log.warn('Day/Night Artwork nicht gefunden');
        return false;
      }
      ...
      log.error('Fehler beim Initialisieren des Day/Night Artwork:', error);
      return false;
    }
  }
  ...
  setupEventListeners() {
    ...
    log.debug('Event-Listener für Day/Night Artwork eingerichtet');
  }
  ...
  startNaturalAnimations() {
    ...
    log.debug('Natürliche Animationen gestartet');
  }
  ...
  destroy() {
    ...
    this.isInitialized = false;
    log.debug('Day/Night Artwork zerstört');
  }
}
...
export async function initializeDayNightArtwork() {
  if (dayNightArtworkState.manager) {
    log.warn('Day/Night Artwork bereits initialisiert');
