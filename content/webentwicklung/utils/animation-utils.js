...
export function resetElementsIn(container) {
  ...
  if (window.enhancedAnimationEngine?.resetElementsIn) {
    ...
    const elements = container.querySelectorAll('[data-animation], [data-animate], .animate-in');
    elements.forEach((el) => {
      el.classList.remove('animate-in', 'is-visible');
    });
  }
  
  log.debug('Container-Animationen zurückgesetzt', { container: container?.tagName });
}
...
export function waitForAnimationEngine(callback, maxAttempts = 50, delay = 100) {
  ...
  const checkEngine = () => {
    if (window.enhancedAnimationEngine && typeof window.enhancedAnimationEngine.scan === 'function') {
      callback();
      return;
    }
    ...
    if (attempts < maxAttempts) {
      ...
      log.warn(`Animation Engine nicht verfügbar nach ${maxAttempts} Versuchen`);
    }
  };
