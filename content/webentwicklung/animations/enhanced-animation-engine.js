...
class EnhancedAnimationEngine {
  ...
  batchTriggerAnimations(entries) {
    ...
    if (this.activeAnimations.size >= maxAnimations) {
      ...
      entries
        .slice(maxAnimations - this.activeAnimations.size)
    ...
    entries.forEach((entry) => {
      ...
      const ratio =
        typeof entry.intersectionRatio === "number"
          ...
          : 0;
      const threshold =
        typeof animationData.threshold === "number"
   ...
   */
  scanForAnimations() {
    this.selectAnimatedElements().forEach((element) =>
      this.scanElement(element)
    );
  }
  ...
  _resolveAnimationType(type) {
    ...
    if (
      !allowedAnimations.includes(baseType) &&
      performanceFallbacks[baseType]
    ) {
