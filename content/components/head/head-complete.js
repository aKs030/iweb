...
(async function loadSharedHead() {
  ...
  try {
      ...
      try {
        fetch("/content/utils/og-image-dimensions.json")
          ...
          .then((map) => {
            ...
            const imgPath = pageData.image.startsWith("/")
              ? pageData.image
              : pageData.image;
    ...
    try {
      ...
      const isDirtyPath =
        window.location.pathname.match(/^\/pages\//i) ||
        window.location.pathname.match(/\/index\.html$/i);
    ...
    try {
      ...
      addIcon(
        `${BASE_URL}/content/assets/img/icons/icon-32.png`,
        "32x32",
        "image/png",
      );
      addIcon(
        `${BASE_URL}/content/assets/img/icons/icon-16.png`,
        "16x16",
        "image/png",
      );
      ...
      if (shortcutEl)
        shortcutEl.setAttribute(
          "href",
          `${BASE_URL}/content/assets/img/icons/favicon.ico`,
        );
      else {
      ...
      let appleIconEl = document.head.querySelector(
        'link[rel="apple-touch-icon"]',
      );
      if (appleIconEl)
        appleIconEl.setAttribute(
          "href",
          `${BASE_URL}/content/assets/img/icons/apple-touch-icon.png`,
        );
      else {
  ...
  const generateSchema = () => {
    ...
    if (faqEntities.length === 0) {
      ...
      const hasBusinessFaqFlag = !!document.querySelector(
