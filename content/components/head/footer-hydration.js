...
const createMenuShell = () => {
  const list = el("ul", { className: "site-menu__list" });
  MENU_ITEMS.forEach(([href, label]) => {
    list.appendChild(el("li", {}, el("a", { href }, label)));
  });
  ...
  fragment.append(
    el(
      ...
      el("span", { className: "site-title" }, "Abdulkerim Sesli"),
      el("span", { className: "site-subtitle show" }, "Portfolio")
    ),
    el("nav", { className: "site-menu", "aria-label": "Hauptnavigation" }, list),
    el(
      "button",
      {
        type: "button",
      ...
      el(
        ...
        el("span", { className: "hamburger-line hamburger-line--top" }),
        el("span", { className: "hamburger-line hamburger-line--middle" }),
        el("span", { className: "hamburger-line hamburger-line--bottom" })
      )
    )
  );
  return fragment;
  ...
  try {
    const run = () => {
      ...
      if (!siteMenu) {
        ...
        siteMenu.replaceChildren(createMenuShell());
      ...
      if (!siteFooter) {
        ...
        siteFooter.replaceChildren(createFooterShell());
