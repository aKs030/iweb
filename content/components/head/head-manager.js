...
const getPageData = () => {
  ...
  const { isEnglish, pageLang, ogLocale } = resolveLanguageContext(preferredLang);
...
const updateBasicMeta = (pageData, pageUrl) => {
  ...
  const twitterDescription = pageData.twitterDescription || ogDescription || pageData.description;
  const pathname = new URL(pageUrl).pathname;
  const keywordList = buildSeoKeywordList(pageData, pathname, extractMainHeadingTerms(document));
  ...
  ogUpdates.forEach(([property, content]) => upsertMeta(property, content, true));
...
async function loadHead() {
  ...
  try {
    ...
    applyCanonicalLinks();
    const pageUrl =
      /** @type {any} */ (document.head.querySelector('link[rel="canonical"]'))?.href ||
