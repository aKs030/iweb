...
import { fetchWithRetry } from "#core/fetch.js";
...
/**
 * @typedef {{ role: string, content: string, timestamp: number }} AgentHistoryEntry
 * @typedef {{
 *   onToken?: (text: string) => void,
 *   onTool?: (event: Record<string, any>) => void,
 ...
 * }} AgentCallPayload
 * @typedef {{ stream?: boolean, signal?: AbortSignal }} AgentCallOptions
 */
...
/**
 * @param {AgentHistoryEntry[]} history
 */
function saveHistory(history) {
  runtimeConversationHistory = Array.isArray(history) ? history.slice(-MAX_HISTORY) : [];
}
 ...
 * @param {AgentCallbacks} [callbacks]
 * @returns {Promise<Record<string, any>|null>}
 */
async function parseSSEStream(response, callbacks = {}) {
  if (!response.body) {
    throw new Error("AI agent stream response has no body.");
  }
...
/**
 * @param {string|undefined|null} finalText
 * @param {string|undefined|null} streamedText
 */
...
/**
 * @param {AgentCallPayload} payload
 * @param {AgentCallbacks} [callbacks]
 * @param {AgentCallOptions} [options]
 */
async function callAgent(payload, callbacks = {}, { stream = true, signal = undefined } = {}) {
  ...
  try {
    if (payload.image) {
      ...
      response = await fetchWithRetry(AGENT_ENDPOINT, {
        fetchOptions: {
          method: "POST",
          body: fd,
          credentials: "omit",
        ...
        retries: 2,
        retryDelay: 320,
        timeout: AGENT_FETCH_TIMEOUT_MS,
  ...
  // ── Error response ──
  if (!response.ok) {
    /** @type {{ retryAfter?: string|number, text?: string }} */
    let body = {};
    try {
  ...
  if (Array.isArray(result.toolCalls)) {
    for (const tc of result.toolCalls) {
      ...
      if (tc.name === "rememberUser" && tc.arguments?.key === "name" && tc.arguments?.value) {
...
export class AIAgentService {
  ...
  validateImage(file) {
    if (!file || !(file instanceof File)) return { valid: false, error: "Keine gültige Datei." };
