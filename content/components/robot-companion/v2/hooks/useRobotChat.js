  ...
  const processUserMessage = useCallback(
    async (text) => {
      ...
      try {
        ...
        const _kb = robotCompanionTexts.knowledgeBase || {};
      ...
      Antworte immer auf DEUTSCH. Fasse dich kurz und prägnant.`;
        ...
        const responseText = await service.generateResponse(
          text,
          (chunk) => {
            setIsTyping(false);
        ...
        setStreamingText('');
        addMessage(
          'Oh, mein Schaltkreis klemmt gerade etwas. Kannst du das wiederholen?',
          'bot',
        );
      } finally {
        setIsTyping(false);
