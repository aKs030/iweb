...
export class RobotIntelligence {
  constructor(robot) {
    ...
    this._idleCheckInterval = this.robot._setInterval(() => this.checkIdle(), 10000);
  }
  ...
  destroy() {
    ...
    if (this.decisionWorker) {
      try {
        this.decisionWorker.removeEventListener("message", this._onWorkerMessage);
  ...
  handleScroll() {
    ...
    if (dt > 100) {
      ...
      if (this._scrollDecayTimer) this.robot._clearTimeout(this._scrollDecayTimer);
