...
export function buildToolResult(name, args, success, message, overrides = {}) {
  ...
  const details = Array.isArray(overrides.details)
    ? overrides.details.filter(detail => detail?.label && String(detail.value || "").trim())
    : [];
  const cta =
    success && overrides.cta !== false
      ? overrides.cta || (config.replayLabel ? { mode: "rerun", label: config.replayLabel } : null)
      : overrides.cta === false
        ? null
        : overrides.cta || null;
  ...
  return {
    ...
    accent: overrides.accent || config.accent || (success ? "utility" : "error"),
    details,
    cta,
    toolArgs: { ...(args || {}) },
  };
