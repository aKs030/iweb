...
function resolveSearchFlightTarget(targetRect) {
  ...
  const windowHeight = typeof globalThis !== "undefined" ? globalThis.innerHeight : 0;
  ...
  const targetTop = Math.max(
    ...
    Math.max(
      MIN_EDGE_OFFSET,
      Math.min(targetRect.bottom + MIN_EDGE_OFFSET, windowHeight - ROBOT_SIZE - MIN_EDGE_OFFSET)
    )
  );
...
export function startSearchAnimation(animation) {
  ...
  const { targetX, targetY } = resolveSearchFlightTarget(targetEl.getBoundingClientRect());
...
export function stopSearchAnimation(animation) {
  ...
  const isNearOrigin = Math.abs(animation.patrol.x) < 0.5 && Math.abs(animation.patrol.y) < 0.5;
...
export function updateSearchAnimation(animation) {
  ...
  if (
    ...
    animation.patrol.x =
      ...
      (animation.searchAnimation.targetX - animation.searchAnimation.startX) * ease;
    animation.patrol.y =
      animation.searchAnimation.startY +
      (animation.searchAnimation.targetY - animation.searchAnimation.startY) * ease;
    ...
    if (animation.robot.dom.svg) {
      ...
      const tilt = isReturning ? 0 : 10 + Math.sin(animation.searchAnimation.hoverPhase) * 2;
      animation.robot.dom.svg.style.transform = `rotate(${tilt}deg)`;
    }

    if (animation.robot.dom.flame) {
      const flameScale = 1 + Math.sin(animation.searchAnimation.hoverPhase * 5) * 0.2;
  ...
  animation.patrol.y =
    animation.searchAnimation.targetY + Math.sin(animation.searchAnimation.hoverPhase) * 12;

  if (animation.robot.dom.flame) {
    const flameScale = 0.8 + Math.sin(animation.searchAnimation.hoverPhase * 3) * 0.15;
