...
export function resolveTypewriterApproachTargetX(typeWriter) {
  ...
  let targetLeft =
    spaceRight >= robotWidth + gap ? twRect.right + gap : twRect.left - robotWidth - gap;
...
const ENTRY_VARIANTS = {
  entryDropIn: {
    duration: 900,
    messages: ["Gelandet! 🛬", "Touchdown! 🪂", "Hallo von oben! 🌤️", "Anflug beendet! ✈️"],
    /**
     * @param {RobotAnimation} animation
     */
    setup(animation) {
  ...
  entryZoomSpin: {
    ...
    setup(animation) {
      ...
      if (animation.robot.dom.floatWrapper) {
        animation.robot.dom.floatWrapper.style.transform = "scale(0.05) rotate(0deg)";
        animation.robot.dom.floatWrapper.style.opacity = "0";
      }
      animation.robot.dom.container.style.transform = "translate3d(0px, 0px, 0)";
    },
    /**
     * @param {RobotAnimation} animation
     * @param {{ t: number }} frameState
  ...
  entrySlideIn: {
    duration: 1200,
    messages: ["Da bin ich! 👋", "Psst… Hey! 🤫", "Alles klar hier? 😊", "Bin eingetroffen! 🚀"],
    /**
     * @param {RobotAnimation} animation
     */
    setup(animation) {
      ...
      animation.robot.dom.container.style.transform = "translate3d(300px, 0px, 0)";
      if (animation.robot.dom.floatWrapper) {
     ...
     */
    frame(animation, { t }) {
      const eased = t === 1 ? 1 : 1 - Math.pow(2, -10 * t) * Math.cos(t * Math.PI * 3);
  ...
  entryBounceAcross: {
    ...
    frame(animation, { t }) {
      ...
      if (animation.robot.dom.floatWrapper) {
        if (nearFloor && t < 0.9) {
          animation.robot.dom.floatWrapper.style.transform = "scaleX(1.15) scaleY(0.85)";
          if (!animation._lastBounceT || Math.abs(t - animation._lastBounceT) > 0.08) {
  ...
  entryPortalOpen: {
    ...
    setup(animation) {
      ...
      animation.robot.dom.container.style.transform = "translate3d(0px, 0px, 0)";
      if (animation.robot.dom.floatWrapper) {
        animation.robot.dom.floatWrapper.style.transform = "scale(0) rotate(0deg)";
        animation.robot.dom.floatWrapper.style.opacity = "0";
      }
    ...
    frame(animation, { t }) {
      ...
      const pt = (t - 0.5) / 0.5;
      const elasticEase = pt === 1 ? 1 : 1 - Math.pow(2, -10 * pt) * Math.cos(pt * Math.PI * 3);
...
export function startRandomEntryAnimation(animation) {
  ...
  const variantKey =
    RANDOM_ENTRY_VARIANT_KEYS[Math.floor(Math.random() * RANDOM_ENTRY_VARIANT_KEYS.length)];
...
export function updateEntryVariant(animation, now) {
  const variantKey = animation.startAnimation.variantKey || animation.startAnimation.phase;
  ...
  if (t >= 1) {
    completeTimedEntryVariant(animation, variant.messages, () => variant.complete?.(animation));
    return true;
  }
