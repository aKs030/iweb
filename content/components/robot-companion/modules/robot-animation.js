...
export class RobotAnimation {
  constructor(robot) {
    ...
    this.defaultMagnifyingGlassTransform = "translate(22, 82) rotate(-45) scale(0.9)";
    /** @type {number | null} */
  ...
  _robotIntersectsTypewriter(targetRect, shrinkX = 10, shrinkY = 6) {
    ...
    const overlapX =
      Math.min(targetRect.right, rRect.right) - Math.max(targetRect.left, rRect.left);
    const overlapY =
      Math.min(targetRect.bottom, rRect.bottom) - Math.max(targetRect.top, rRect.top);
  ...
  _startHomeIntroSequence() {
    ...
    this.startAnimation.duration = compact ? 720 : this.entryConfig.homeIntroDurationMs;
  ...
  startTypeWriterKnockbackAnimation() {
    ...
    if (this._isHomeContext()) {
      ...
      const fallbackDelay = this._isCompactViewport() ? 1100 : this.entryConfig.homeIntroFallbackMs;
  ...
  updateStartAnimation() {
    ...
    if (this.startAnimation.phase === "settleIn") {
      ...
      const duration = this.startAnimation.duration || this.entryConfig.homeIntroDurationMs;
      ...
      if (t >= 1) {
        ...
        this.setAvatarState({ moving: false, dashing: false });
        this.pausePatrol(this.entryConfig.homeIntroPauseMs + Math.random() * 600);
        this.startPatrol();
        return;
      }
    ...
    if (this.startAnimation.phase === "naturalApproach") {
      ...
      if (now > this.startAnimation.naturalNextPauseAt) {
        ...
        this.startAnimation.naturalNextPauseAt = now + pauseDuration + 1200 + Math.random() * 1800;
      ...
      if (this._naturalSpeedFrame === 0 || this._naturalSpeedCached === undefined) {
    ...
    if (this.startAnimation.phase === "pause") {
      if (now >= this.startAnimation.pauseUntil) {
        ...
        if (typeWriter) {
          ...
          this.robot.collisionModule._playTextCollisionEffect(typeWriter, twRect);
        }
    ...
    if (this.startAnimation.phase === "knockback") {
      ...
      const eased = t === 1 ? 1 : 1 - Math.pow(2, -10 * t) * Math.cos(t * Math.PI * 2.5);
      const endX = this.startAnimation.knockbackEndX || 0;
      this.patrol.x =
        this.startAnimation.knockbackStartX + (endX - this.startAnimation.knockbackStartX) * eased;
  ...
  startPatrol() {
    this.patrol.active = true;
    if (this.robot.dom && this.robot.dom.container) this.robot.dom.container.style.opacity = "1";
    this.robot._requestAnimationFrame(this.updatePatrol);
  }

  updatePatrol() {
    if (document.hidden) {
      this.robot._setTimeout(() => this.robot._requestAnimationFrame(this.updatePatrol), 500);
      return;
    }

    if (!this.patrol.active || this.startAnimation.active || this.searchAnimation.active) {
      return;
    }
    ...
    const robotWidth = 80;
    const initialLeft =
      (typeof globalThis !== "undefined" ? globalThis.innerWidth : 0) - 30 - robotWidth;
    ...
    if (typeWriter && twRect) {
      const avoidedTypewriter = this._maintainHomeTypewriterClearance(twRect, maxLeft);
    ...
    if (this.robot.dom.flame) {
      const flameIntensity =
        this.startAnimation && this.startAnimation.active ? 0.9 : dashActive ? 0.85 : 0.75;
  ...
  pausePatrol(ms) {
    ...
    if (this.robot.dom.thinking && Math.random() < 0.3) {
      ...
      this.robot._setTimeout(() => {
        if (this.robot.dom.thinking) this.robot.dom.thinking.style.opacity = "0";
      }, ms * 0.6);
    }
    this.robot._setTimeout(() => {
      this.patrol.isPaused = false;
