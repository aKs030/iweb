...
export class RobotCollision {
  ...
  markDecorativeElements() {
    ...
    if (this._lastDecorativeTagUpdate && now - this._lastDecorativeTagUpdate < 3000) {
      return;
    }
    this._lastDecorativeTagUpdate = now;

    const nodes = document.querySelectorAll(DECORATIVE_IGNORE_SELECTORS.join(","));
    nodes.forEach(el => {
  ...
  getScannableCandidates(candidates) {
    ...
    const maxChecks = candidates.length > 100 ? 16 : candidates.length > 60 ? 22 : 28;
    const selected = [];
  ...
  scanForCollisions() {
    ...
    const minScanIntervalMs =
      obstacleCount > 100 ? 95 : obstacleCount > 60 ? 70 : obstacleCount > 30 ? 50 : 35;

    // Adaptive throttling based on current obstacle density.
    const now = performance.now();
    if (this._lastCollisionCheck && now - this._lastCollisionCheck < minScanIntervalMs) return;
    ...
    const candidates = this.getScannableCandidates(this.getCandidatesForCollisionScan(robotRect));
  ...
  triggerCollisionReaction(obs, obsRect) {
    ...
    if (type === "dizzy") {
      ...
      if (this.robot.dom.eyes) {
        ...
        if (this.robot.dom.pupils) this.robot.dom.pupils.forEach(p => (p.style.display = "none"));
        if (this.robot.dom.lids) this.robot.dom.lids.forEach(l => (l.style.display = "none"));

        const dizzyGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
        dizzyGroup.classList.add("dizzy-eyes-temp");
        ["M35,38 L45,46 M45,38 L35,46", "M55,38 L65,46 M65,38 L55,46"].forEach(d => {
          const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
          path.setAttribute("d", d);
          path.setAttribute("stroke", "var(--robot-svg-stroke)");
          path.setAttribute("stroke-width", "3");
          dizzyGroup.appendChild(path);
        });
        ...
        this.robot._setTimeout(() => {
          ...
          if (this.robot.dom.pupils) this.robot.dom.pupils.forEach(p => (p.style.display = ""));
          if (this.robot.dom.lids) this.robot.dom.lids.forEach(l => (l.style.display = ""));
  ...
  _playTextCollisionEffect(typeWriter, twRect) {
    ...
    const effects = ["collision-shake", "collision-glitch", "collision-scatter", "collision-flash"];
    ...
    const impactX = Math.max(twRect.left, Math.min(twRect.right, rRect.left + rRect.width / 2));
    const impactY = Math.max(twRect.top, Math.min(twRect.bottom, rRect.top + rRect.height / 2));
