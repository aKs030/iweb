...
const ACTION_PROMPTS = {
  ...
  [ROBOT_ACTIONS.SCROLL_FOOTER]: "Scrolle bitte zum Footer und bestaetige kurz auf Deutsch.",
  [ROBOT_ACTIONS.TOGGLE_THEME]: "Wechsle bitte das Theme und bestaetige kurz auf Deutsch.",
  [ROBOT_ACTIONS.SEARCH_WEBSITE]: "Hilf mir bei der Website-Suche und frage nach dem Suchbegriff.",
  ...
  [ROBOT_ACTIONS.CLOSE_SEARCH]: "Schliesse bitte die Suche und bestaetige kurz.",
  [ROBOT_ACTIONS.SCROLL_TOP]: "Scrolle bitte ganz nach oben und bestaetige kurz.",
  [ROBOT_ACTIONS.COPY_CURRENT_URL]: "Kopiere den aktuellen Seitenlink und bestaetige kurz.",
  [ROBOT_ACTIONS.CLEAR_CHAT]: "Loesche den Chatverlauf und bestaetige kurz auf Deutsch.",
};
...
export class RobotChat {
  constructor(robot) {
    ...
    // Runtime chat state used across robot modules.
    this.isOpen = false;
    this.isTyping = false;
  ...
  async applyUserName(rawName) {
    ...
    const existingPrompt = this.robot.dom.messages?.querySelector(".message--name-prompt");
    existingPrompt?.remove();

    this.addMessage(`Alles klar, ich merke mir deinen Namen als ${normalized}.`, "bot");
  ...
  async requestMemoryAction(action, payload = {}) {
    ...
    const data = await fetchJSON(AI_AGENT_USER_ENDPOINT, {
      fetchOptions: {
        method: "POST",
        headers,
        credentials: "omit",
    ...
    if (data?.error || data?.success === false) {
      const text = data?.text || data?.error || "Profil-Request fehlgeschlagen.";
  ...
  removeMemoryManagerCard() {
    this.robot.dom.messages?.querySelector(".message--memory-manager")?.remove();
  }
  ...
  createMemoryRow(memory, onEdit, onForget) {
    ...
    const updatedLabel = updatedAt ? new Date(updatedAt).toLocaleString("de-DE") : "-";
    const expiresLabel = expiresAt ? new Date(expiresAt).toLocaleDateString("de-DE") : "-";
  ...
  renderMemoryManagerCard() {
    ...
    toolbar.append(
      this.createMemoryManagerButton("Neu laden", () => void this.showMemoryManager()),
      this.createMemoryManagerButton("Name ändern", () => this.renderNamePrompt()),
      this.createMemoryManagerButton("Disconnect", () => void this.disconnectProfile(), "danger")
    );
    ...
    const loadedAtLabel = loadedAt ? new Date(loadedAt).toLocaleString("de-DE") : "-";
    transparencyList.append(
      this.createTransparencyItem("Aktive User-ID", state.profile?.userId || "-"),
      this.createTransparencyItem("Profilstatus", String(state.profile?.status || "unbekannt")),
      ...
      state.memories.forEach(memory => {
        memories.appendChild(
          this.createMemoryRow(
            ...
            () =>
              void this.forgetMemoryEntry(String(memory?.key || ""), String(memory?.value || ""))
          )
        );
      });
    }
    ...
    if (Array.isArray(state.recoveryCandidates) && state.recoveryCandidates.length) {
  ...
  setMemoryManagerState(nextState = {}) {
    ...
    this.memoryManagerState = {
      ...previous,
      ...nextState,
      memories: Array.isArray(nextState.memories) ? nextState.memories : previous.memories || [],
  ...
  async resolveProfilesByName(recoveryName) {
    try {
      ...
      const candidates = Array.isArray(data?.recovery?.candidates) ? data.recovery.candidates : [];
      const autoCandidateUserId = String(data?.recovery?.autoCandidateUserId || "").trim();
      if (autoCandidateUserId) {
        this.setMemoryManagerState({
          recoveryName,
          recoveryCandidates: candidates,
          feedback: data.text || "Passendes Profil erkannt. Aktiviere es direkt...",
          feedbackTone: "info",
        });
      ...
      this.setMemoryManagerState({
        ...
        recoveryCandidates: candidates,
        feedback:
          data.text || (candidates.length > 0 ? "Profile gefunden." : "Keine Profile gefunden."),
        feedbackTone: "info",
      });
    } catch (error) {
      this.setMemoryManagerState({
  ...
  async disconnectProfile() {
    try {
      ...
      this.addMessage(data?.text || "Profil getrennt. Bitte Namen neu setzen.", "bot");
      this.renderNamePrompt();
    } catch (error) {
      this.setMemoryManagerState({
  ...
  renderQuickLinks() {
    if (!this.robot.dom.messages) return;
    if (this.robot.dom.messages.querySelector(".message--quick-actions")) return;
    ...
    quickLinks.forEach(item => {
      ...
      button.addEventListener("click", () => this._executeQuickAction(item.action));
      track.append(button);
    });
  ...
  _applyVisualChatState(newState, options = {}) {
    ...
    if (newState) {
      ...
      const isInitialOpenWithoutHistory =
        this.robot.dom.messages.children.length === 0 && this.history.length === 0;
  ...
  async handleUserMessage() {
    ...
    try {
      ...
      const response = await this._streamAgentResponse(async (agentService, onChunk, signal) => {
        if (hasPendingImage) {
      ...
      this.addMessage("Fehler bei der Verbindung. Bitte erneut versuchen.", "bot");
    } finally {
      this.isResponding = false;
      this.syncComposerState();
    }
  ...
  showToolCallResults(toolResults) {
    ...
    for (const result of toolResults) {
      const indicator = this.robot.domBuilder.createToolCallIndicator(result.name, result.message);
  ...
  addImageMessage(text, imageFile) {
    ...
    this.history = this.historyStore.append(this.history, {
      role: "user",
      text: text ? `[Bild: ${imageFile.name}] ${text}` : `[Bild: ${imageFile.name}]`,
      timestamp,
    });
  }
  ...
  handleImageUpload(file) {
    ...
    const inputArea = this.robot.dom.inputArea || document.getElementById("robot-input-area");
    ...
    if (this.robot.dom.input) {
      this.robot.dom.input.placeholder = "Beschreibe das Bild oder sende es direkt...";
  ...
  updateStreamingMessage(messageEl, text) {
    ...
    if (textSpan) {
      setSanitizedHTML(textSpan, MarkdownRenderer.parse(text));
      this.scrollToBottom();
    }
  }
  ...
  finalizeStreamingMessage(messageEl, { interrupted = false } = {}) {
    ...
    const timestamp = Number.parseInt(messageEl.dataset.timestamp || "", 10) || Date.now();
  ...
  async handleSummarize() {
    ...
    try {
      ...
      const response = await this._streamAgentResponse((agentService, onChunk, signal) =>
  ...
  renderMessage(text, type = "bot", skipParsing = false, timestamp = Date.now()) {
    const msg = document.createElement("div");
    ...
    if (type === "user") {
      ...
      if (skipParsing) {
        ...
        setSanitizedHTML(msg, String(text || ""));
      } else {
        // Parse markdown (MarkdownRenderer sanitizes output)
        setSanitizedHTML(msg, MarkdownRenderer.parse(String(text || "")));
      }
    }
  ...
  async _routeToAI(actionKey, prompt) {
    ...
    this.robot._setTimeout(() => this.robot.dom.avatar.classList.remove("nod"), 650);
  ...
  syncComposerState() {
    ...
    const canSend = !this.isTyping && !this.isResponding && (hasText || hasPendingImage);
