...
import { executeCopyCurrentUrl, executeSetTheme } from "./tool-handlers/ui-tools.js";
...
function confirmToolExecution(toolCall) {
  const title = String(toolCall?.meta?.confirmTitle || "").trim() || DEFAULT_CONFIRM_TITLE;
  const message =
...
export function executeTool(toolCall) {
  ...
  try {
    if (toolDefinition?.execution === "server") {
      return buildToolResult(name, args, false, `Tool "${name}" wird serverseitig verarbeitet.`, {
        title: "Server-Tool",
    ...
    if (!isClientToolAvailable(name)) {
      return buildToolResult(name, args, false, "Tool aktuell nicht verfügbar.", {
    ...
    if (shouldConfirmToolExecution(toolCall) && !confirmToolExecution(toolCall)) {
