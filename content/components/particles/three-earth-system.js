	...
	_updateFrame(totalTime, delta, capabilities) {
		...
		if (this.cardManager) {
			const featuresCameraTransitioning =
				...
				Boolean(this.cameraManager?.transition?.active);
	...
	_updateEarthForSection(sectionName, allowModeSwitch) {
		...
		if (this.directionalLight && this.ambientLight) {
			...
			this.directionalLight.intensity =
				sectionLight.sunIntensity ?? lightCfg.SUN_INTENSITY;
			this.ambientLight.intensity =
				sectionLight.ambientIntensity ?? lightCfg.AMBIENT_INTENSITY;
			this.ambientLight.color.setHex(
				sectionLight.ambientColor ?? lightCfg.AMBIENT_COLOR,
			);
		}
	}
