  ...
  return new Promise((resolve) => {
    ...
    input
      ...
      .on('finish', () => {
        ...
        console.log(
          `✅ Compressed: ${filePath}`,
          `${(originalStats.size / 1024).toFixed(2)} KiB → ${(compressedStats.size / 1024).toFixed(2)} KiB (${ratio}% reduction)`
        );
        resolve(true);
      })
      .on('error', (error) => {
...
async function compress() {
  ...
  console.log('\n' + '='.repeat(60));
  console.log(`✨ Compression complete: ${successCount} successful, ${failureCount} failed`);
  ...
  if (successCount > 0) {
    ...
    console.log('');
    console.log('For Cloudflare Workers or modern servers, add this to your headers:');
    console.log('');
