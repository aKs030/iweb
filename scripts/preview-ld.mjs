...
globalThis.location = { origin: 'https://abdulkerimsesli.de', pathname: '/about/', href: 'https://abdulkerimsesli.de/about/' };
...
const doc = {
  ...
  head: { querySelector: (sel) => {
      // Simulate dateCreated meta when requested
      if (/meta\[property="article:published_time"\]|meta\[name="dateCreated"\]|meta\[property="og:published_time"\]/.test(sel)) {
        return { getAttribute: (k) => (k === 'content' ? '2025-12-31T12:00:00Z' : null) };
      }
      return null;
    } },
  querySelector: (sel) => {
    if (sel === 'main')
      return {
        cloneNode: () => ({ innerText: 'Über mich — Entwickler, Fotograf. Portfolio, Kontakt.' }),
        innerText: 'Über mich — Entwickler, Fotograf. Portfolio, Kontakt.',
      };
    if (sel === 'time[datetime][pubdate], time[datetime][itemprop="dateCreated"], time[datetime][data-created]') return null;
    return null;
  },
  querySelectorAll: (sel) => {
    if (sel === 'main img, .profile-photo, .avatar')
      return [{ src: 'https://abdulkerimsesli.de/content/assets/img/icons/icon-512.png' }];
...
const BRAND_DATA = {
  ...
  sameAs: ['https://github.com/aKs030', 'https://linkedin.com/in/abdulkerimsesli'],
  followersCount: 5400,
  likesCount: 12000,
  postsCount: 234,
};
...
(async () => {
  try {
    const graph = generateSchemaGraph(pageData, `${BASE_URL}/about/`, BASE_URL, BRAND_DATA, BUSINESS_FAQS, doc);
