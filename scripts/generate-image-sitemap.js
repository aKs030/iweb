...
const META_PATH = path.resolve(__dirname, '../content/assets/img/og/og-images-meta.json');
...
async function gatherPages(meta) {
  ...
  const blogData = await readFileSilent(path.join(PROJECT_ROOT, 'pages', 'blog', 'blog-data.js'));
  if (blogData) {
...
function buildImageTag(m, fallbackOnly = false) {
  ...
  if (m.fallback) images.push({ loc: absoluteUrl(m.fallback), caption: '', title: '' });
  // include a webp if present
  if (metaSources.webp) {
    // pick the largest
    const largest = metaSources.webp.reduce((acc, cur) => (cur.width > acc.width ? cur : acc), metaSources.webp[0]);
    images.push({ loc: absoluteUrl(largest.url), caption: '', title: '' });
  }
  // include jpg if present and not same as fallback
  if (metaSources.jpg) {
    const largestJ = metaSources.jpg.reduce((acc, cur) => (cur.width > acc.width ? cur : acc), metaSources.jpg[0]);
    if (!images.find((i) => i.loc === absoluteUrl(largestJ.url))) images.push({ loc: absoluteUrl(largestJ.url), caption: '', title: '' });
  }
  return images;
}
...
async function readExistingExternalImages() {
  ...
  for (const b of urlBlocks) {
    ...
    const external = images.filter((u) => u.startsWith('http') && !u.includes('www.abdulkerimsesli.de'));
    if (external.length) {
...
async function generate() {
  ...
  for (const [pageUrl, data] of Object.entries(pages)) {
    ...
    for (const img of data.images) {
      ...
      if (found) {
        ...
        imagesForPage.push({ loc: img.src.startsWith('http') ? img.src : absoluteUrl(img.src), caption: '', title: '' });
      }
    }
    // dedupe
    const dedup = Array.from(new Map(imagesForPage.map((i) => [i.loc, i])).values());
    if (!dedup.length) continue;
    writtenPages.add(pageUrl);

    body += `  <url>\n    <loc>${pageUrl}</loc>\n    <lastmod>${today()}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.7</priority>\n`;
    for (const img of dedup) {
      body += `    <image:image>\n      <image:loc>${img.loc}</image:loc>\n      <image:caption>${(data.title || '').replace(/&/g, '&amp;')}</image:caption>\n      <image:title>${(data.title || '').replace(/&/g, '&amp;')}</image:title>\n      <image:license>${BASE_URL}/#image-license</image:license>\n    </image:image>\n`;
    }
    body += `  </url>\n\n`;
  }
  ...
  if (!writtenPages.has(BASE_URL + '/')) {
    const homeKeyCandidates = Object.keys(meta).filter((k) => k.startsWith('og-home'));
  ...
  if (externalBlocks.length) {
    ...
    for (const b of externalBlocks) body += `  <url>${b.split('</url>')[0]}  </url>\n\n`;
  }
