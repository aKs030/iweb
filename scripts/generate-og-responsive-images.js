...
(async () => {
  ...
  for (const f of images) {
    try {
      ...
      if (!meta[base].originalWidth || (info.width || 0) > (meta[base].originalWidth || 0)) {
      ...
      const pipeline = info.width && info.width >= target ? img.resize(target) : img.clone();
      await pipeline.webp({ quality: FORMATS[0].options.quality }).toFile(outPath);

      // set canonical sources/fallback
      meta[base].sources['webp'] = [{ url: `/content/assets/img/og/${outName}`, width: target }];
