...
async function minifyFile(filePath) {
  ...
  try {
    ...
    console.log(
      `✅ Minified: ${filePath} (${(stats.size / 1024).toFixed(2)} KiB)`
    );
    return true;
  } catch (error) {
...
async function optimizeThreeJS() {
  ...
  if (!fs.existsSync(threeSourcePath)) {
    console.log(
      "ℹ️  Local three.module.js not found. Skipping Three.js optimization."
    );
    console.log(
      '   Hint: Download it with: curl -L "https://cdn.jsdelivr.net/npm/three@0.155.0/build/three.module.js" -o content/vendor/three/three.module.js'
    );
    return false;
  }
  ...
  try {
    ...
    console.log(
      `✅ Three.js optimized: ${sizeMiB.toFixed(2)} MiB (estimate: ${(
        (stats.size * 0.57) /
        1024
      ).toFixed(0)} KiB minified)`
    );
...
async function build() {
  ...
  console.log("\n" + "=".repeat(60));
  console.log(
    `✨ Build complete: ${successCount} successful, ${failureCount} failed`
  );
