...
(async ()=>{
  ...
  if(!key){ console.error('ERROR: Please pass the key via YT_KEY env var'); process.exit(2); }
  ...
  const recon = Buffer.from(partA, 'base64').toString('utf8') + Buffer.from(partB, 'base64').toString('utf8');
  if(recon !== key){ console.error('ERROR: Reconstruction mismatch'); process.exit(3); }
