...
function analyzeThreeJSUsage() {
  const threeEarthDir = path.join(process.cwd(), 'content/components/particles/earth');
  const mainFile = path.join(process.cwd(), 'content/components/particles/three-earth-system.js');
  ...
  files.forEach(file => {
    const filePath = file === path.basename(mainFile) 
      ? mainFile
...
function main() {
  ...
  Array.from(usedFeatures).sort().forEach(feature => {
    console.log(`  • ${feature}`);
  });
  ...
  Object.entries(savings).forEach(([method, stats]) => {
    ...
    console.log(`${method}`);
    console.log(`  Size: ${stats.size.toFixed(0)} KiB (${stats.savings}% smaller)`);
  ...
  console.log('   https://threejs.org/docs/#manual/en/introduction/Building-lightweight-apps\n');
}
