  ...
  try {
    const run = function () {
      ...
      if (!document.getElementById('footer-trigger-zone')) {
        ...
        if (siteFooter?.parentNode) {
          siteFooter.parentNode.insertBefore(trigger, siteFooter);
        } else {
  ...
  try {
    const getStylesForPath = () => {
      ...
      if (p === '/') {
        return base.concat([
          ...
          '/components/particles/three-earth.css',
          '/pages/home/section3.css',
        ]);
      }
      if (p.includes('/projekte')) {
        return base.concat(['/components/particles/three-earth.css']);
      }
      return base;
    };

    const SCRIPTS = [
      { src: '/main.js', module: true, preload: false },
      { src: '/components/menu/menu.js', module: true },
    ];
    ...
    const deferNonCriticalAssets = () => {
      try {
        ...
        schedule(() => {
          upsertScript({
            src: '/components/robot-companion/robot-companion.js',
            module: true,
          });
        });
      } catch (err) {
